...
function openProductModal(product){
  modalImg.src = product.images && product.images.length ? product.images[0] : "";
  modalTitle.textContent = product.title;
  modalPrice.textContent = "RD$ " + (product.price_cents/100).toFixed(2);
  ...
  sizes.forEach(s=>{
    ...
    b.textContent = s;
    b.onclick = ()=> {
      document.querySelectorAll("#modalSizes button").forEach(x=>x.classList.remove("active"));
  ...
  colors.forEach(c=>{
    ...
    b.textContent = c;
    b.onclick = ()=> {
      document.querySelectorAll("#modalColors button").forEach(x=>x.classList.remove("active"));
      b.classList.add("active");
    };
...
document.addEventListener("click", e=>{
  ...
  if(card && card.dataset.id){
    const prod = window.products.find(p=>p.id===card.dataset.id);
